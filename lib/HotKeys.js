...
class HotKeys extends Component {
   ...
   */
  getContextValue() {
    if (!this._memoizedContext ||
      this._memoizedContext.hotKeyParent !== this ||
      this._memoizedContext.hotKeyMap !== this.__hotKeyMap__)
  ...
  render() {
    ...
    const clonedFirstChild = firstChild
      ? cloneElement(firstChild, {
        ref: (ref) => {
          this._ref = ref;

          if (typeof firstChild.ref === 'function') {
            firstChild.ref(ref);
          } else if (firstChild.ref) {
            firstChild.ref.current = ref;
          }
        }
      })
      : undefined;
    ...
    return (
      <FocusTrap {...props} onFocus={this.onFocus} onBlur={this.onBlur}>
        <HotKeysContext.Provider value={this.getContextValue()}>
          {clonedFirstChild}
          {restChildren}
